<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-white">Editar Log de Treino</h1>
                        <p class="mt-1 text-gray-100">Atualize as informações do log de treino</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.workout_logs.update', $workoutLog->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        ...
                        <select id="workout_id" name="workout_id"
                            ...
                            @foreach($workouts as $workout)
                                <option value="{{ $workout->id }}" {{ $workoutLog->workout_id == $workout->id ? 'selected' : '' }}>{{ $workout->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="student_id" name="student_id"
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" {{ $workoutLog->student_id == $student->id ? 'selected' : '' }}>{{ $student->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input id="date" name="date" type="date" value="{{ \Carbon\Carbon::parse($workoutLog->date)->format('Y-m-d') }}"
                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div class="flex justify-end">
                    ...
                    <button type="submit"
                            class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Atualizar</button>
                </div>
            </form>
            </div>
        </div>
