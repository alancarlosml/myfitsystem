...
<div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
    ...
    <h2 class="text-4xl font-extrabold dark:text-white">
        ...
        <input type="hidden" name="establishment_id" value="{{ $category->establishment->id }}">
    </h2>
    @else
    ...
    <div class="sm:col-span-2">
        ...
        <textarea id="description" rows="8"
                    ...
                    placeholder="Descrição" name="description">{{ $category->description ?? old('description') }}</textarea>
    </div>
    <div class="sm:col-span-2">
        <label class="inline-flex items-center cursor-pointer">
            <input type="checkbox" value="1" class="sr-only peer" name="active" {{ old('active', $category->active ?? false) ? 'checked' : '' }}>
