<x-app-layout>
    ...
    <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-orange-600 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-white">Estabelecimentos</h1>
                    <p class="mt-1 text-amber-100">Gerencie todos os estabelecimentos do sistema</p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                        <p class="text-xs text-amber-200">Total de estabelecimentos</p>
                        <p class="text-2xl font-bold text-white">{{ $establishments->count() }}</p>
                    </div>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <x-alert-success />
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-amber-100 text-sm font-medium">Total de Estabelecimentos</p>
                            <p class="text-3xl font-bold mt-1">{{ $establishments->count() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-green-100 text-sm font-medium">Estabelecimentos Ativos</p>
                            <p class="text-3xl font-bold mt-1">{{ $establishments->where('active', 1)->count() }}</p>
                        </div>
                        <div class="bg-white/20 rounded-lg p-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-red-100 text-sm font-medium">Estabelecimentos Pendentes</p>
                            <p class="text-3xl font-bold mt-1">{{ $establishmentsWithPendingContracts ?? 0 }}</p>
                            <p class="text-xs text-red-200 mt-1">com contratos pendentes ou vencidos</p>
                        </div>
                        <div class="bg-white/20 rounded-lg p-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-yellow-100 text-sm font-medium">Este Mês</p>
                            <p class="text-2xl font-bold mt-1">{{ $establishments->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
                            <p class="text-xs text-yellow-200 mt-1">novos estabelecimentos</p>
                        </div>
                        <div class="bg-white/20 rounded-lg p-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
                <form method="GET" action="{{ route('admin.establishments.index') }}" id="filterForm" x-data="filterData" x-cloak>
                    <!-- Linha de Busca -->
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                        <div class="w-full md:w-1/2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="simple-search" x-model="filters.search"
                                       class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500"
                                       placeholder="Buscar estabelecimentos por nome, endereço ou cidade..."
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="submit"
                                    class="inline-flex items-center px-4 py-2.5 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 rounded-lg transition-colors">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            @if($types->count() > 0)
                                <div>
                                    ...
                                    <select name="type" id="type" x-model="filters.type"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-amber-500 dark:focus:border-amber-500">
                                        <option value="">Todos</option>
                    ...
                    <div x-show="hasActiveFilters()" x-transition
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                                ...
                                <template x-if="value && key !== 'search'">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200">
                                        ...
                                        <button type="button" @click="removeFilter(key)" 
                                                class="ml-2 inline-flex items-center justify-center w-4 h-4 rounded-full hover:bg-amber-200 dark:hover:bg-amber-700 transition-colors">
                            ...
                            <template x-if="filters.search">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                                    ...
                                    <button type="button" @click="filters.search = ''; applyFilters()" 
                                            class="ml-2 inline-flex items-center justify-center w-4 h-4 rounded-full hover:bg-yellow-200 dark:hover:bg-yellow-700 transition-colors">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                                        </svg>
                                    </button>
                            ...
                            <button type="button" @click="clearFilters()"
                                    class="ml-auto text-xs text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300 font-medium">
                                Limpar todos
                            </button>
                        </div>
                    </div>
        ...
        <script>
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('filterData', () => ({
                    showFilters: @json(!empty(request('status')) || !empty(request('type')) || !empty(request('created_from')) || !empty(request('created_to'))),
                    filters: {
                    ...
                    hasActiveFilters() {
                        return this.filters.status || this.filters.type || this.filters.created_from || this.filters.created_to || this.filters.search;
                    },
                    getFilterLabel(key, value) {
                        ...
                        const labels = {
                            ...
                            type: 'Tipo: ' + value.charAt(0).toUpperCase() + value.slice(1),
