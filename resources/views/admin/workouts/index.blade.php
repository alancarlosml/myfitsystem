    ...
    <div class="bg-gradient-to-r from-orange-600 via-red-600 to-pink-600 text-white">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-3xl font-bold text-white">Treinos</h1>
                    <p class="mt-1 text-red-100">Gerencie todos os treinos do sistema</p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                        <p class="text-xs text-red-200">Total de treinos</p>
                        <p class="text-2xl font-bold text-white">{{ $workouts->count() }}</p>
                    </div>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 py-8">
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-orange-100 text-sm font-medium">Total de Treinos</p>
                            <p class="text-3xl font-bold mt-1">{{ $workouts->count() }}</p>
                        </div>
                ...
                <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold mt-1">{{ $workouts->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
                            <p class="text-xs text-red-200 mt-1">novos treinos</p>
                        </div>
                        <div class="bg-white/20 rounded-lg p-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-pink-500 to-pink-600 rounded-xl shadow-lg p-6 text-white">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-pink-100 text-sm font-medium">Esta Semana</p>
                            <p class="text-3xl font-bold mt-1">{{ $workouts->where('created_at', '>=', now()->startOfWeek())->count() }}</p>
                            <p class="text-xs text-pink-200 mt-1">treinos criados</p>
                        </div>
                        <div class="bg-white/20 rounded-lg p-3">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6">
                <form method="GET" action="{{ route('admin.workouts.index') }}" id="filterForm" x-data="filterData" x-cloak>
                    <!-- Linha de Busca -->
                        ...
                        <div class="w-full md:w-1/2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="search" id="simple-search" x-model="filters.search"
                                       ...
                                       placeholder="Buscar treinos por aluno, exercício ou instrutor..."
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
                <div class="overflow-x-auto" x-data="{ workouts: {{ @json_encode($workouts) }}, selectAll: false }">
                    @include('admin.workouts.partials.table', ['workouts' => $workouts])
                </div>
            </div>
        </div>
    </section>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('filterData', () => ({
                showFilters: @json(!empty(request('created_from')) || !empty(request('created_to'))),
                filters: {
                ...
                hasActiveFilters() {
                    return this.filters.created_from || this.filters.created_to || this.filters.search;
                },
