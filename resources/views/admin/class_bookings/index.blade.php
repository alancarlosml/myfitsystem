                ...
                <div class="w-full border-2 border-gray-400 dark:border-gray-700 rounded-lg p-4">
                    <div class="md:p-8 p-5 dark:bg-gray-800 bg-white">
                        <div class="px-4 mb-4 flex items-center justify-between">
                            <span  tabindex="0" class="focus:outline-none font-bold dark:text-gray-100 text-gray-800 text-2xl">Julho 2024</span>
                                                    ...
                                                    @if ($dayData['date']->month == $month)
                                                        @if (!empty($dayData['events']))
                                                            ...
                                                            <div class="flex items-center justify-center w-full rounded-full cursor-pointer">
                                                                <a role="link" tabindex="0"  data-date="{{ $dayData['date']->format('Y-m-d') }}" data-today="{{ $dayData['is_today'] ? 'true' : 'false' }}" class="text-base w-8 h-8 flex items-center justify-center font-medium text-blue-500 @if ($dayData['is_today']) bg-blue-100 rounded-full @endif">{{ $dayData['date']->day }}</a>
                                                            </div>
                                                        @else
                                                            <!-- Dia sem eventos no mês atual -->
                ...
                $('a[data-date]').click(function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            $('a[data-date]').removeClass('focus:ring-2 focus:ring-offset-2 focus:ring-blue-700 focus:bg-blue-500 hover:bg-blue-200 bg-blue-100 rounded-full');
                
                            // Adiciona a classe ao elemento clicado
                            $this.addClass('focus:ring-2 focus:ring-offset-2 focus:ring-blue-700 focus:bg-blue-500 hover:bg-blue-200 bg-blue-100 rounded-full');
                                ...
                                $.each(response.events, function(index, event) {
                                    ...
                                    var eventHtml = `
                                        <div class="border-b pb-4 border-gray-400 border-dashed mt-3">
                                            <div class="flex items-center justify-between">
                                                <div class="w-full">
                                                    <p class="text-xs font-light leading-3 text-gray-500 dark:text-gray-300">${startTime} - ${endTime}</p>
                                                    <a tabindex="0" class="focus:outline-none text-lg font-medium leading-5 text-gray-800 dark:text-gray-100 mt-2">${description}</a>
                                                    <p class="text-xs font-light leading-3 text-gray-500 dark:text-gray-300">Instrutor</p>
                                                </div>
                                                <div class="flex items-center">
